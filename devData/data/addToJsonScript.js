...
const addQunatity = () => {
  const agroJsonApiData = fs.readFileSync(
    `${__dirname}/agroJsonApi.json`,
    'utf-8'
  );
  const parsedAgroJsonApiData = JSON.parse(agroJsonApiData);

  parsedAgroJsonApiData.map(el => {
    // const newProduct = Object.assign({ Quantity: 0 });
    // parsedAgroJsonApiData.push(newProduct);
    fs.writeFile(
      `${__dirname}/agroData.json`,
      JSON.stringify(el.assign({ quantity: 0 })),
      error => {
        console.log('data can not!');
      }
    );
